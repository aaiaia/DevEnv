...
if [ -d $VIM_DIR/colors ]; then    # Directory is exist
# Backup previous one
echo "Directory is exist, backup to ~/.vim/color_bak_$CURRENT_TIME"
mv $VIM_DIR/colors $VIM_DIR/colors_bak_$CURRENT_TIME
elif [ -L $VIM_DIR/colors ]; then  # Symbolic Links is exist
# Backup previous one
